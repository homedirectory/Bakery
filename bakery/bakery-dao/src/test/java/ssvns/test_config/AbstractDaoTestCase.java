...
import ssvns.test_config.UniversalConstantsForTesting;
...
public abstract class AbstractDaoTestCase extends AbstractDomainDrivenTestCase {
    ...
    private void setupPerson(final User.system_users defaultUser, final String emailDomain) {
        if (!useSavedDataPopulationScript()) {
            ...
            save(new_(Person.class, defaultUser.name()).setActive(true).setUser(su)
                      .setDesc("Person who is a user")
                      .setEmail(defaultUser + "@" + emailDomain));
        }
    }
